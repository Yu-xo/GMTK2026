...
func _perform_check() -> void:
	var melee_batches: int = melee_wave.size()
	var range_batches: int = range_wave.size()
	var tank_batches: int = tank_wave.size()
	var time_batches: int = spawn_time.size()
	var array_comparison: Array[int] = [melee_batches, range_batches, tank_batches, time_batches]
	var shortest_int: int = 0

	if spawn_time.size() >= 1:
		melee_wave.resize(time_batches)
		range_wave.resize(time_batches)
		tank_wave.resize(time_batches)
	
	else:
		print("There must be at least one spawn time")
